...
class FCT(L.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        loss = (self.loss_fn(pred_y[2], y) * 0.57 + self.loss_fn(pred_y[1], down1) * 0.29 + self.loss_fn(pred_y[0],
                                                                                                         down2) * 0.14)
...
# a utiliser dans le fichier main segmentation si assez de ram 32 gb
import argparse
...
def parse_args():
    ...
    parser.add_argument("--amp", default=True, help="use automatic mix precision, default True")

    # train parameters
    parser.add_argument('--batch_size', default=4, type=int, help='batch size, default size 1')
    parser.add_argument('--max_epoch', default=20, type=int, help='the max number with epoch')
    ...
    parser.add_argument('--min_lr', default=6e-6, help='min dynamic learning rate')
    parser.add_argument('--lr_scheduler', default='ReduceLROnPlateau')
    parser.add_argument('--resume', action='store_true')
    parser.add_argument('--new_param', action='store_true', help='use new param when resume from checkpoint')

    parser.add_argument('--colab', action='store_true')
    # network parameters

    # Parse only known arguments and ignore the rest
